...
///
/// Both passes run here, like every other golden suite. This file was
/// CI-only for a while because the readable baselines are rendered on
/// Linux and it was written on a macOS host, which could not produce
/// one; `make goldens-linux` renders them in a container now, so the
/// host somebody develops on no longer decides which gates they can run.
library;
...
void main() {
  group('later components', () {
                          ...
                          WaxTable<_Job>(
                            ...
                            columns: <WaxColumn<_Job>>[
                              ...
                              WaxColumn<_Job>(
                                label: 'State',
                                text: (job) => job.state,
                                cell: (context, job) => CodecChip(job.state),
                              ),
                              WaxColumn<_Job>(
                                label: 'Items',
                                numeric: true,
                                text: (job) => '${job.items}',
                                cell: (context, job) => Text('${job.items}'),
                              ),
    ...
    // Here rather than beside the other card goldens because captions
    // came later, which is what this file collects. What it locks is a
    // measurement: hiding them must not change the cell a card had.
    goldenTest(
