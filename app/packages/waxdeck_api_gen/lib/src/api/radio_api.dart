...
class RadioApi {
  ...
  /// The cover this server holds for whatever the station last announced, for the case the announced track is not in this library. Draw it on the full-screen player face; the deck bar keeps the station logo on purpose, because a bar whose picture changed every few minutes would read as the station changing.  Three rungs sit behind radio artwork and this endpoint serves the middle two. The first is &#x60;nowPlayingItemPid&#x60; above: a match against this library, which costs no network and is the common answer for a listener playing a station whose music they own.  On its miss comes the picture the **station itself announced**, where it announced one: many carry a per-track cover (or a channel logo) in the &#x60;StreamArtwork&#x60; or &#x60;StreamUrl&#x60; field of the same in-stream metadata the title arrives in. That is not a third-party lookup - the URL came down the stream this listener is already receiving - so &#x60;radioExternalArt&#x60; does not gate it, and neither does switching that setting off drop what has already been fetched. The bytes are fetched through the SSRF-guarded client the station logo uses, capped, and typed by sniffing them.  Last comes MusicBrainz and the Cover Art Archive, resolved from the announced artist and title. That rung is **off by default** (&#x60;radioExternalArt&#x60; in the server settings): it sends a string a station chose from a self-hosted server to a third party, which is the operator&#39;s call to make. With it off, a station that announces no picture of its own leaves this endpoint answering 404 and the client draws the station mark, which is a designed state rather than a gap.  Never blocking, and a client should expect that. The lookup is started by the play-info poll and runs against a service paced at roughly one request per second, so the first poll after a title changes answers 404 here and a later one answers the image - on the same fifteen-second cadence the play-info contract already asks for. A miss is remembered for a day rather than forever, because a track released this week can have no archive entry today and one tomorrow, and a service that could not be reached is remembered for minutes only.  The bytes go through the same raster-only check the station logo does, decided by inspecting them rather than by trusting the upstream &#x60;Content-Type&#x60;, and carry the same hardening headers. 
  ///
  /// Parameters:
  /// * [pid] - Type-prefixed PID (e.g. `tr-01JZX5N8QW3F4V9T2B7KD3M9R6`).
