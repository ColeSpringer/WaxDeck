...
abstract class Prefs implements Built<Prefs, PrefsBuilder> {
  ...
  /// Stations whose segments this user does not scrobble, while the account scrobbles radio at all. The account-wide `radioScrobbleOptOut` is the blunt answer for a listener who wants none of it; this is the household that reports its music stations and not its talk ones, whose titles happen to parse.  Read after the account-wide switch, so an account that has opted out is silent whatever this holds. Unordered as far as the server is concerned - it is a set, and the order a client writes is the order it reads back, because nothing sorts it.  The same shape and the same rules as `radioFavorites`: entries are station PIDs (`rs-...`), stored in the canonical upper-case form the pattern declares, not resolved on write, capped at 64, and absent when nothing is muted rather than stored as `[]`. A station another household member deleted leaves its pid here, holding a slot of that cap until somebody unmutes it - the same trade `radioFavorites` makes, and for the same reason: failing a whole preference write over one departed station would be worse. Unmuting the last station drops the field. 
  @BuiltValueField(wireName: r'radioScrobbleMutedStations')
  BuiltList<String>? get radioScrobbleMutedStations;
  ...
  /// Stop scrobbling radio. Listeners with a scrobble connection have their radio segments reported by default, which is right for a music station whose stream titles are honest and wrong for a talk station whose titles happen to parse. Opting out silences radio only; library listening is unaffected. The whole dial: silencing one station is `radioScrobbleMutedStations`, which is read only when this is unset. 
...
class _$PrefsSerializer implements PrimitiveSerializer<Prefs> {
  ...
  Iterable<Object?> _serializeProperties(
    ...
    if (object.radioScrobbleMutedStations != null) {
      yield r'radioScrobbleMutedStations';
      yield serializers.serialize(
        object.radioScrobbleMutedStations,
        specifiedType: const FullType(BuiltList, [FullType(String)]),
      );
    }
