...
/// * [nowPlayingItemPid] - A library track this server matched `nowPlaying` to, present only when that track has cover art, so a full-screen player can draw the song's own cover instead of the station logo. Absent is the common case and is not a failure, so a client must fall back to the station's artwork rather than showing a gap. The match is a best-effort text lookup against the catalog and is never authoritative: it does not mean this server is playing that file, and it is deliberately not used for scrobbling, which reports what the station announced. 
