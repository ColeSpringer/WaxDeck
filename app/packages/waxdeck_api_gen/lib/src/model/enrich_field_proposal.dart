...
/// One field an enrichment provider would fill. `lyrics` carries the full text (LRC when the provider answered timed lines); `genre` the joined, normalized genre scalar; the `fields` and `book` wants propose each scalar they can fill as its own row, and each of those two commits as one edit, so all its rows name the same provider.  Every row is fill-when-empty and lock-respecting: enrichment never replaces a value someone else set, which is what makes the injected providers safe to run beside MusicBrainz matching. Matching is authoritative for identity and locks what it writes, so these fill only what nothing has claimed. 
///
/// Properties:
