...
administrators), and UnifiedPush registrations for
push-transported events. The shapes (keyset pagination, typed
PIDs, the structured error model, relative media URLs) are
...
- Errors are always the `Error` schema. `code` is a stable machine-readable
  ...
  could not be reached; try again later), and `service-unreachable`
