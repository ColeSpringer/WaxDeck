...
WebSocket channel in `api/events.md`). The shapes (keyset
pagination, typed PIDs, the structured error model, relative
media URLs) are load-bearing from day one.
...
- Identifiers (`pid`) are type-prefixed ULIDs (`tr-` track, `al-` album,
  ...
  `pe-` player endpoint, `ps-` playback session).
...
- Errors are always the `Error` schema. `code` is a stable machine-readable
  ...
  integration, that this server is not running), and
  `endpoint-offline` (the target player endpoint is not currently
  connected; refresh the endpoint list).
