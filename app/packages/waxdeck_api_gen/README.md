...
ingest, and library scanning. The shapes (keyset pagination, typed
...
- Errors are always the `Error` schema. `code` is a stable machine-readable
  ...
  matches the file on disk; re-request play-info), and
  `catalog-maintenance` (the catalog is temporarily handed to a
