...
import { rectAtRest, typeOnce, wheelIntoReach } from '../gestures';
...
export class Admin extends Surface {
  ...
  /// Fills the libraries screen's add form and submits it, retried against
  /// the POST since the table above can move the button. A duplicate name
  /// answers 409, which counts as created. `typeOnce`: one deadline only.
  async addLibrary(who: { name: string; path: string }): Promise<void> {
    const page = this.ctx.page;
    const submit = page.locator(sem(SemanticsIds.librarySubmit));
    await expect(async () => {
      await typeOnce(page, page.locator(sem(SemanticsIds.libraryName)), who.name);
      await typeOnce(page, page.locator(sem(SemanticsIds.libraryPath)), who.path);
      await wheelIntoReach(page, submit);
      // Armed and awaited together, so a click that throws orphans no wait.
